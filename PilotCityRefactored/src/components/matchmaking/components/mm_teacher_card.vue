<template>
  <div class="card container">
    <div class="one d-flex flex-row h-25">
      ...
      <h2 class="card-title">{{classroom.course_name}}</h2>
      <!-- Computer Science AP -->
    ...
    <div class="two d-flex flex-row h-50">
      ...
      <div class="five w-75">
        <h4 class="card-subtitle-text">{{classroom.teacher_name}}</h4>
        <!-- Anthony Keithley -->
        ...
        <h4 class="card-subtitle-text">{{teacher.school_distrct}}</h4>
        <!-- San Leandro Unified -->
        <h4 class="card-subtitle-text">{{teacher.school_name}}</h4>
        <h4 class="card-subtitle-text">
          <span v-for="(field,index) in Object.keys(teacher.school_address)" :key="index">
            <span>{{teacher.school_address[field]}}</span>
